...
static rbt * get_node_by_key(int key,int * di)
{
	rbt * node = root;
	if(node == NIL_NODE){
		*di = 0;
...
static rbt * new_rbt_node(int key)
{
...
static void print_rbt_node(rbt * node,char i)
{
	char * lpr;
	if(node == NIL_NODE){
